...
import type { TranscriptData } from "./meeting-ui.types";
...
function asTranscriptEntry(value: unknown): TranscriptEntry {
    if (!isRecord(value)) return { text: String(value ?? "") };
...
export default function TranscriptTab({ transcriptData, meetingStatus }: TranscriptTabProps) {
    ...
    if (isRecord(raw) && typeof raw.cleaned === "string") {
        entries = [{ text: raw.cleaned }];
    } else if (Array.isArray(raw)) {
        entries = raw.map(asTranscriptEntry);
    } else if (typeof raw === "string") {
        entries = [{ text: raw }];
    } else if (isRecord(raw) && "raw" in raw) {
