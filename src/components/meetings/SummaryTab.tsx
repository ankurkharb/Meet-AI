...
import type { SummaryData } from "./meeting-ui.types";
...
export default function SummaryTab({ summaryData, meetingStatus }: SummaryTabProps) {
    ...
    const sentimentKey = s?.sentiment?.overall ?? "neutral";
    const sentiment = sentimentColors[sentimentKey] || sentimentColors.neutral;
    const keyPoints = s?.keyPoints ?? [];
    const actionItems = s?.actionItems ?? [];
    const decisionsMade = s?.decisonsMade ?? [];
    const speakerHighlights = s?.speakerHighlights ?? [];
            ...
            {s?.sentiment && (
                <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
                    ...
                    <div style={{ backgroundColor: "#ede9fe", border: "2px solid #000", padding: "0.75rem 1.25rem" }}>
                        ...
                        <p style={{ fontWeight: 800, color: "#6d28d9", fontSize: "1.5rem" }}>{keyPoints.length}</p>
                    </div>
                    <div style={{ backgroundColor: "#fef3c7", border: "2px solid #000", padding: "0.75rem 1.25rem" }}>
                        ...
                        <p style={{ fontWeight: 800, color: "#92400e", fontSize: "1.5rem" }}>{actionItems.length}</p>
                    </div>
                </div>
            )}
            ...
            {keyPoints.length > 0 && (
                <div style={{ border: "2px solid #000", backgroundColor: "#fff" }}>
                    ...
                    <div style={{ padding: "1rem" }}>
                        {keyPoints.map((pt, i) => (
                            <div key={i} style={{ display: "flex", gap: "0.75rem", padding: "0.5rem 0", borderBottom: i < keyPoints.length - 1 ? "1px solid #e5e7eb" : "none" }}>
                ...
                <div style={{ border: "2px solid #000", backgroundColor: "#fff" }}>
                    ...
                    <div style={{ padding: "1rem" }}>
                        {actionItems.map((item, i) => (
                            <div key={i} style={{ display: "flex", gap: "0.75rem", alignItems: "flex-start", padding: "0.5rem 0", borderBottom: i < actionItems.length - 1 ? "1px solid #e5e7eb" : "none" }}>
                ...
                <div style={{ border: "2px solid #000", backgroundColor: "#fff" }}>
                    ...
                    <div style={{ padding: "1rem" }}>
                        {decisionsMade.map((d, i) => (
                            <div key={i} style={{ display: "flex", gap: "0.75rem", padding: "0.5rem 0", borderBottom: i < decisionsMade.length - 1 ? "1px solid #e5e7eb" : "none" }}>
