...
import type { CreateMeetingInput, MeetingStatusType, UpdateMeetingInput } from "./meetings.types";
import { nanoid } from "nanoid";

export interface GetManyParams {
    userId: string;
    agentId?: string;
    status?: "all" | MeetingStatusType;
    page?: number;
    pageSize?: number;
}
...
export const meetingsService = {
    ...
    async getMany({ userId, search, page = 1, pageSize = 10, status, agentId }: GetManyParams & { search?: string }) {
        ...
        if (status && status !== "all") {
            conditions.push(eq(meeting.status, status));
        }
