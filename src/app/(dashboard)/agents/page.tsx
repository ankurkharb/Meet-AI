...
import type { Agent, AgentsFilterParams, CreateAgentInput, UpdateAgentInput } from "@/modules/agents";

const agentStatusFilters = ["all", "active", "inactive", "archived"] as const;

function isAgentStatusFilter(value: string): value is AgentsFilterParams["status"] {
    return agentStatusFilters.includes(value as AgentsFilterParams["status"]);
}
...
function AgentsPageContent() {
    ...
    const selectedStatus = isAgentStatusFilter(status) ? status : "all";

    const { data, isLoading, error, refetch } = trpc.agents.getMany.useQuery({
        search, status: selectedStatus, page, pageSize: 10,
    });
