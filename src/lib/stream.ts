...
function getStreamServerClient(): StreamClient {
    const apiKey = process.env.NEXT_PUBLIC_STREAM_API_KEY;
    const apiSecret = process.env.STREAM_API_SECRET;

    if (!apiKey || !apiSecret) {
        throw new Error("NEXT_PUBLIC_STREAM_API_KEY and STREAM_API_SECRET are required to use video features.");
    }
...
export function generateStreamToken(userId: string): string {
    ...
    return getStreamServerClient().createToken(userId, expirationTime, issuedAt);
}
...
export async function createAgentUser(agentId: string, agentName: string) {
    try {
        await getStreamServerClient().upsertUsers([
            {
                id: `agent_${agentId}`,
                name: agentName,
                role: "user",
...
export async function addAgentToCall(callId: string, agentId: string, agentName: string) {
    try {
        ...
        // Get the call
        const call = getStreamServerClient().video.call("default", callId);
